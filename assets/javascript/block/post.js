...
( function( $ ) {
	$( function() {
		...
		$( '#fuse_block_fields_meta' ).on( 'click', '#block-add-sub-field', function() {
			const template = wp.template( 'field-repeater' ),
				data = { uid: new Date().getTime() },
				row = $( template( data ) ),
		...
		$( '#fuse_block_properties_meta .block-properties-icon-select span' ).on( 'click', function() {
			const svg = $( 'svg', this ).clone();
			$( '#fuse_block_properties_meta .block-properties-icon-select span.selected' ).removeClass( 'selected' );
			$( this ).addClass( 'selected' );
			$( '#block-properties-icon' ).val( $( this ).data( 'value' ) );
			$( '#block-properties-icon-current' ).html( svg );
		} );

		$( '#fuse_block_properties_meta .block-properties-category' ).on( 'change', function() {
